...
// ── Filter taxonomy ────────────────────────────────────────────────────────
// The dropdown *contents* are live: they're derived from the taxonomy published
// fact-checks actually carry (`lib/data/filter-options.ts`). What stays here is
// (a) how each dimension is presented (label + icon) and (b) a curated fallback
// used only in degraded mode, when Hasura is unreachable — the same
// `?? fallback` convention as the rest of `lib/*-content.ts`.

/** Region → subject scheme `countries` (ISO3). PesaCheck's core coverage. */
const FALLBACK_REGIONS: FilterOption[] = [
...
const FALLBACK_LANGUAGES: FilterOption[] = [
...
const FALLBACK_TOPICS: FilterOption[] = [
...
/**
 * Dropdown contents for degraded mode only — used when the live taxonomy read
 * fails (`getFilterOptions()` throws). In normal operation the options come
 * from Superdesk, so adding a language/region/topic there needs no edit here.
 */
export const FALLBACK_FILTER_OPTIONS: FilterOptions = {
  region: FALLBACK_REGIONS,
  language: FALLBACK_LANGUAGES,
  topic: FALLBACK_TOPICS,
};

/**
 * How each filter dimension is presented (dropdown label + icon). Presentation
 * only — the options themselves are fetched. Kept in dimension order; the icons
 * are React components, so this is imported directly by the client components
 * rather than passed through props.
 */
export const FILTER_META: {
  ...
  { dimension: "region", label: "Region", icon: FiGlobe },
  { dimension: "language", label: "Language", icon: LuLanguages },
  { dimension: "topic", label: "Topic", icon: LuMessageCircleWarning },
];
